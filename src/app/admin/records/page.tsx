...
// 参加者ごとに記録件数・アンケート件数・最終記録日時を表示し、
// クリックで個別ページに遷移する。
// このページへのアクセスは src/middleware.ts の Basic 認証で保護されている。

import Link from "next/link";
import { listAllSnapMeta, listAllSurveyMeta } from "@/lib/db-server";
import {
  listAllParticipants,
...
export default async function AdminRecordsPage() {
  ...
  const [snapRows, surveyRows] = await Promise.all([
    listAllSnapMeta(),
    listAllSurveyMeta(),
  ]);
  ...
  for (const r of snapRows) {
    snapCount.set(r.participantId, (snapCount.get(r.participantId) ?? 0) + 1);
  }
  const surveyCount = new Map<string, number>();
  for (const r of surveyRows) {
    surveyCount.set(
      r.participantId,
      (surveyCount.get(r.participantId) ?? 0) + 1,
    );
  }
  ...
  for (const r of snapRows) {
    ...
    if (!prev || r.takenAt > prev) lastMap.set(r.participantId, r.takenAt);
  }
  ...
  return (
    <main className="min-h-dvh p-6 max-w-3xl mx-auto">
      ...
      <p className="text-sm opacity-70 mb-6">
        参加者をクリックすると、点眼記録とアンケート結果が表示されます。
        顔写真は端末外に送信されていません。
      </p>
      ...
      <ul className="divide-y rounded-xl border overflow-hidden">
        {participants.map((p: ParticipantEntry, i: number) => {
          ...
          const sn = snapCount.get(p.participantId) ?? 0;
          const sv = surveyCount.get(p.participantId) ?? 0;
            ...
            <li key={p.participantId}>
              ...
              >
                ...
                <div className="flex items-center gap-4 text-sm opacity-80">
                  <span className="tabular-nums">記録 {sn}</span>
                  <span className="tabular-nums">回答 {sv}</span>
                  {last && (
      ...
      <p className="mt-6 text-xs opacity-60">
        合計 記録 {snapRows.length} 件 / 回答 {surveyRows.length} 件
      </p>
    </main>
  );
}
